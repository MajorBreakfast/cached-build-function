...
class CachedBuildFunction extends Function () {
   ...
   * @return {*}
   */
  // static hashInput (...args) { return args }
  ...
  _run (args, blockRun) {
    // Hash the input
    const hashInputFn = this.constructor.hashInput
    ...
    const promise = (async () => {
      ...
      emitter.emit('checkedCache', { cacheHit })
      emitter.emit(cacheHit ? 'cacheHit' : 'cacheMiss')
   ...
   *   the extra properties `on` and `emitter`. Use this if you're already
  ...
  flush (options) {
    ...
    Promise.all(queue.map(item => item.checkedCache))
      .then(cacheResults => {
        ...
        emitter.emit('checkedCache', { count, cacheHitCount, cacheMissCount })
